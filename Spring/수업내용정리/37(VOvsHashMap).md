...
```
### 테이블을 JOIN할 시, 데이터 타입을 Map으로 지정한다.
```
xml에서 테이블들을 서로 JOIN할 시, vo 클래스에 필드변수를 지정해줘야 한다.
ex) emp 테이블에 컬럼수 20개, dept 테이블에 컬럼수 80개라고 가정,
해당 vo 클래스에 100개의 필드변수를 지정해줘야 한다 => 일일히 입력해야 하는 번거로움 발생

map으로 리스트를 받으면 필드변수를 지정해주지 않아도 된다. 
