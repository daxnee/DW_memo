...
```java
    ...
    int y = 5;
    int result = x + y ;
//  └2번          └1번 으로 실행 (대입연산자를 기준으로 읽는 순서가 다름. 오른쪽에서 왼쪽, 위에서 아래)
   // => 변수를 이용해서 결과 값을 받음
...
```java
    ...
    출력값 : 40
```
    ? 기준으로 왼쪽의 조건이 맞으면 40, 거짓이면 30을 출력
    변수 = 조건 ? true:false

    - 실무 예시
    변수 = (회원이 로그인했을 경우) ? home : login
    로그인 성공했으면 home 로그인 실패했으면 login
